...
path = "/Users/Mikazuki/Document/GitHub/rgb/train_jpg"
    ...
    if os.path.exists(pic_path[i]):
        ...
        for row in range(2):
            for col in range(2):
                ...
                pic1_block = pic1[(row * rows//2): ((row+1) * rows//2 - 1),
                                  (col * col//2): ((col+1) * col//2 - 1)]
                lbp = feature.local_binary_pattern(
                    pic1, n_points, radius, 'uniform')
                lbp2 = lbp.astype(np.int32)
                max_bins = int(lbp2.max() + 1)
                train_hist, _ = np.histogram(
                    lbp2, normed=True, bins=max_bins, range=(0, max_bins))
...
X_train, X_test, y_train, y_test = model_selection.train_test_split(
    Data, label1, test_size=0.2, random_state=0)
# train_data = Data[0:7]
# train_label = label1[0:7]
# test_data = Data[8:9]
# test_label = label1[8:9]
svr_rbf = svm.SVR(kernel='rbf', C=1e3, gamma=0.1)
# .fit(train_data, train_label).score(test_data,test_label)
model = multiclass .OneVsRestClassifier(svr_rbf, -1)
